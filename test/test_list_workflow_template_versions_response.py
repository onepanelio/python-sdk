# coding: utf-8

"""
    namespace.proto

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    The version of the OpenAPI document: version not set
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import core.api
from core.api.models.list_workflow_template_versions_response import ListWorkflowTemplateVersionsResponse  # noqa: E501
from core.api.rest import ApiException

class TestListWorkflowTemplateVersionsResponse(unittest.TestCase):
    """ListWorkflowTemplateVersionsResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test ListWorkflowTemplateVersionsResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = core.api.models.list_workflow_template_versions_response.ListWorkflowTemplateVersionsResponse()  # noqa: E501
        if include_optional :
            return ListWorkflowTemplateVersionsResponse(
                count = 56, 
                workflow_templates = [
                    core.api.models.workflow_template.WorkflowTemplate(
                        created_at = '0', 
                        uid = '0', 
                        name = '0', 
                        version = 56, 
                        manifest = '0', 
                        is_latest = True, 
                        is_archived = True, )
                    ]
            )
        else :
            return ListWorkflowTemplateVersionsResponse(
        )

    def testListWorkflowTemplateVersionsResponse(self):
        """Test ListWorkflowTemplateVersionsResponse"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
